...
# Load a skill-local .env (API keys etc.) if present, so child scripts
# (polish_transcript.py, fetch_stock.py, …) inherit ANTHROPIC_API_KEY /
# PEXELS_API_KEY without the caller having to export them. Values already
...
# Locate a Chromium for Remotion. Some sandboxed/CI environments block
# Remotion's browser download host (remotion.media) — in that case Remotion
# can reuse an already-installed chrome-headless-shell. Honor an explicit
# REMOTION_BROWSER_EXECUTABLE override first; otherwise auto-detect a
# Playwright-managed headless_shell. If neither is found, leave it empty and
# let Remotion download its own browser (the normal path on unrestricted nets).
REMOTION_BROWSER_FLAG=""
if [ -n "${REMOTION_BROWSER_EXECUTABLE:-}" ] && [ -x "${REMOTION_BROWSER_EXECUTABLE}" ]; then
  REMOTION_BROWSER_FLAG="--browser-executable=${REMOTION_BROWSER_EXECUTABLE}"
elif [ -n "${PLAYWRIGHT_BROWSERS_PATH:-}" ]; then
  _hs="$(ls "${PLAYWRIGHT_BROWSERS_PATH}"/chromium_headless_shell-*/chrome-linux/headless_shell 2>/dev/null | head -n1)"
  [ -x "${_hs:-}" ] && REMOTION_BROWSER_FLAG="--browser-executable=${_hs}"
fi
...
(cd "$REMOTION_DIR" && npx --no-install remotion render src/index.ts EditedVideo "$OUT_PATH" \
  ...
  ${REMOTION_BROWSER_FLAG:+"$REMOTION_BROWSER_FLAG"} \
  --timeout=120000)
